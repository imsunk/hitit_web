							...
							<div id="collapse" class="panel-collapse collapse in">
								<div class="panel-body">
									<input name="B_NAME" class="form-control" type="text"
										maxlength="10" value="<%=board.getB_NAME() %>">
								</div>
							</div>
						</div>
							...
							<div id="collapse" class="panel-collapse collapse in">
								<div class="panel-body">
									<input name="B_SUBJECT" class="form-control" type="text"
										size="50" maxlength="100" value="<%=board.getB_SUBJECT() %>" />
								</div>
							</div>
						</div>
							...
							<div id="collapse" class="panel-collapse collapse in">
								<div class="panel-body">
									<textarea name="B_CONTENT" class="form-control" cols="67"
										rows="15"><%=board.getB_CONTENT() %></textarea>
								</div>
							</div>
						</div>
						<input type="hidden" name="B_NUM" value="<%=board.getB_NUM() %>" />
						<input type="hidden" name="category"
							value="<%=board.getCATEGORY() %>" />
						<%if(!(board.getB_FILE()==null)){ %>
						파일 첨부 &nbsp;&nbsp;<%=board.getB_FILE() %>
						<%} %>

						<div class="col-xs-2">
							<div class="col-xs-3">
								<a href="javascript:modifyboard()"
									class="btn btn-block btn-lg btn-primary">수정</a>
							</div>
							<div class="col-xs-3">
